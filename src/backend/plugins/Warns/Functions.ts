...
export const DelWarn = async (id: string, guild: Guild) => {
    ...
    cases.forEach((x) => {
        x.cases.forEach((v) => {
            if(v.type === "warn" && v.id === id) { return (index = { userId: x.userId, index: x.cases.indexOf(v) }); }
        })
    })
    if (!index) { return; }
    let userCases = await Cases.findOne({ guildId: guild.id, userId: index.userId })
    userCases.cases.splice(index.index, 1); userCases.save();
    userCases = await Cases.findOne({
      guildId: guild.id,
      userId: index.userId,
    });
    if(!userCases.cases || userCases.cases.length < 1) { await Cases.findOneAndRemove({userId: index.userId, guildId: guild.id })}
    return index;
}
